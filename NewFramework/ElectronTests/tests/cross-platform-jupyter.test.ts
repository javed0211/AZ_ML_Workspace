 ...
 */
test.describe('Cross-Platform Jupyter Notebook Runner', () => {
  let vscodeProcess: any;
  let testWorkspace: string;
  let platform: string;
  let vscodeCommand: string;
  ...
  test('should run existing Jupyter notebook from any folder on any OS', async () => {
    ...
    vscodeProcess = spawn(vscodeCommand, launchArgs, {
      stdio: 'pipe',
      detached: false,
    ...
    let processCheckCommand: string;
    switch (platform) {
      case 'win32':
    ...
    spawn(openCommand, [notebookPath], {
      stdio: 'pipe',
      env: process.env as { [key: string]: string },
    ...
    try {
      ...
      expect(finalVersion).toMatch(/^\d+\.\d+\.\d+$/);
  ...
  test('should handle different notebook formats and encodings', async () => {
    ...
    for (const [filename, content] of Object.entries(notebooks)) {
      ...
      expect(parsedContent.nbformat).toBe(4);
  ...
  test('should run existing notebook from any specified folder', async () => {
    ...
    if (fs.existsSync(NOTEBOOK_FOLDER_PATH)) {
      ...
      const openCommand = platform === 'win32' ? 'code.cmd' : 'code';
      spawn(openCommand, [fallbackPath], {
        stdio: 'pipe',
        env: process.env as { [key: string]: string },
        shell: platform === 'win32'
      });
